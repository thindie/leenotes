...
import com.example.thindie.leenotes.data.di.NotesRepositoryModule
import dagger.Component

@Component(
    dependencies = [DependenciesProvider::class],
    modules = [NotesScreenViewModelModule::class, ViewModelFactoryModule::class, NotesRepositoryModule::class]
)
@NoteFeatureScope
interface NotesScreenComponent {
    companion object {
        fun init(dependenciesProvider: DependenciesProvider): NotesScreenComponent {
            return DaggerNotesScreenComponent.factory().create(dependenciesProvider)
        }
    }
